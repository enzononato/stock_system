...
export function ProcessStatusBadge({ process }: { process: OffboardingProcess }) {
  ...
  return (
    <Badge
      ...
      className={cn(
        "font-medium",
        process.rejected && "border-destructive/25 bg-destructive/10 text-destructive",
        isConcluded(process) &&
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Gestor: {process.gestor} · Desligamento previsto: {formatDate(process.dataPrevista)}
            {process.setor ? ` · ${process.setor}` : ""}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">Motivo: {process.motivo}</p>
        </div>
        <div className="shrink-0 text-left sm:text-right">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
            Etapa {process.currentStep} de {STEPS.length}
          </p>
          <p className="mt-0.5 text-xs font-medium text-foreground">
            {current?.label ?? "Concluído"}
          </p>
        </div>
      </div>
      <div className="mt-4">
        ...
        <StepTimeline process={process} />
      </div>
      {process.rejected ? (
        <p className="mt-3 rounded-md border border-destructive/25 bg-destructive/5 p-2.5 text-xs text-destructive">
          Rejeitado por {process.rejected.by}: {process.rejected.reason}
        </p>
      ) : null}
...
export function StepTimeline({ process }: { process: OffboardingProcess }) {
  return (
    <ol
      ...
      aria-label="Etapas do desligamento"
    >
      {STEPS.map((s) => {
        const done = process.currentStep > s.n;
        const current = process.currentStep === s.n && !process.rejected;
        return (
          <li key={s.key}>
            <span
              title={`${s.n}. ${s.label} — ${s.owner}`}
              className={cn(
                "inline-flex w-full items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium sm:w-auto",
                done &&
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <Label htmlFor={`${id}-note`} className="text-xs text-muted-foreground">
          {noteLabel}
          {requireNote ? " (obrigatória)" : " (opcional)"}
        </Label>
        <Textarea
          id={`${id}-note`}
          value={note}
      ...
      <Button
        size="sm"
        disabled={disabled || !checked || (requireNote && !note.trim())}
        onClick={() => {
          onConfirm(note.trim());
          setNote("");
          setChecked(false);
...
export function InfoChecklist({
  ...
  values: Partial<Record<string, StepState>>;
  onChange: (key: string, state: StepState) => void;
  disabled?: boolean;
}) {
  return (
        ...
        return (
          <li key={item.key} className="flex items-start gap-2">
            <Checkbox
              ...
              disabled={disabled}
              onCheckedChange={(v) =>
                onChange(item.key, { done: v === true, at: new Date().toISOString() })
              }
            />
