    ...
    <div className="page-container-dense space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 items-start">
        ...
        <div className="rounded-[6px] border border-border bg-surface p-4 space-y-4">
          {/* Tabs de Filtro */}
          <div className="grid grid-cols-2 gap-1 p-1 rounded-[4px] bg-surface-alt border border-border">
            <button
              type="button"
              onClick={() => setFilterTab("pendentes")}
              className={`py-1.5 text-caption font-semibold rounded-[2px] transition-colors ${
                filterTab === "pendentes"
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-[4px] border cursor-pointer transition-all ${
                      isSelected
                        ? "border-foreground bg-surface-alt font-medium"
                        : "border-border hover:bg-muted/30"
                    }`}
                  >
                    <div className="flex items-center justify-between text-caption">
                      <span className="font-mono font-semibold text-foreground">#{item.id}</span>
                      <span className="text-muted-foreground font-mono">{formatDate(item.date_issued)}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-caption text-muted-foreground mt-1">
                      <span className="truncate">{item.tipo} {item.brand}</span>
        ...
        {selectedItem ? (
          <div className="rounded-[6px] border border-border bg-surface p-8 space-y-6 shadow-sm">
            ...
            <div className="border-b border-border pb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="font-mono text-caption text-muted-foreground mt-0.5">
                  REF: TR-{String(selectedItem.id).padStart(6, "0")} • EMISSÃO: {formatDate(selectedItem.date_issued)}
                </p>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 rounded-[4px] border border-border bg-surface-alt p-4 text-body-sm">
                <div>
                  <span className="text-caption text-muted-foreground block">Custodiante / Colaborador</span>
                  <span className="font-semibold text-foreground">{selectedItem.assigned_to || "—"}</span>
                  <span className="text-caption font-mono text-muted-foreground block mt-0.5">
                    CPF: {selectedItem.cpf || "—"}
                  </span>
                </div>
                <div>
                  <span className="text-caption text-muted-foreground block">Lotação e Unidade</span>
                  ...
                  <span className="text-caption text-muted-foreground block mt-0.5">
                    Setor: {selectedItem.setor || "—"} {selectedItem.cargo ? `• ${selectedItem.cargo}` : ""}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 rounded-[4px] border border-border bg-surface-alt p-4 text-caption">
                <div>
                  <span className="text-muted-foreground block">ID Patrimonial</span>
                  <span className="font-mono font-semibold text-foreground">#{selectedItem.id}</span>
                </div>
                <div>
            ...
            {/* Cláusula 3: Cláusulas Contratuais Resumidas */}
            <div className="space-y-3 pt-2">
              ...
              <div className="space-y-2 text-caption text-muted-foreground leading-relaxed border-l-2 border-border pl-4">
                <p>
                  <strong>3.1. Finalidade:</strong> O equipamento acima discriminado é de propriedade exclusiva da Revalle e é cedido em comodato exclusivamente para execução das atividades funcionais.
                </p>
                <p>
                  <strong>3.2. Conservação:</strong> O colaborador compromete-se a zelar pelo perfeito estado de funcionamento e conservação do bem, comunicando imediatamente ao departamento de TI qualquer falha, dano ou extravio.
                </p>
                <p>
                  <strong>3.3. Restituição:</strong> O colaborador compromete-se a restituir o equipamento nas mesmas condições em que o recebeu no caso de rescisão contratual, transferência ou solicitação expressa da empresa.
                </p>
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-border">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4">
                <div className="border-t border-foreground/40 pt-2 text-center text-caption">
                  <span className="font-semibold text-foreground block">{selectedItem.assigned_to || "Colaborador"}</span>
                  <span className="text-muted-foreground">Custodiante Responsável</span>
                </div>
                <div className="border-t border-foreground/40 pt-2 text-center text-caption">
                  <span className="font-semibold text-foreground block">Departamento de TI / Patrimônio</span>
          ...
          <div className="rounded-[6px] border border-border bg-surface p-12 text-center text-caption text-muted-foreground">
