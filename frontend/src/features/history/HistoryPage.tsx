...
function OperationBadge({ op }: { op?: string | undefined }) {
  ...
  if (op.includes("Empréstimo"))
    return (
      <Badge className="badge-info" variant="outline">
        {op}
      </Badge>
    );
  if (op.includes("Devolução"))
    return (
...
function AttachmentCell({
  ...
  if (entry.termo_assinado_anexo)
    attachments.push({ key: entry.termo_assinado_anexo, label: "Termo" });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          icon={ArrowDownLeft}
        />
        <KpiCard label="Estornos" value={estornos} hint="Na página atual" icon={RotateCcw} />
      </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {reversingEntry?.peripheral_id != null &&
                ` do periférico #${reversingEntry.peripheral_id}`}
              {reversingEntry?.operador && ` (operador: ${reversingEntry.operador})`}. Esta ação não
              pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
