...
export function StockPage() {
  ...
  const columns: Column<Item>[] = [
    ...
    { key: "actions", header: "Ações", hideOnMobile: true, align: "right", cell: (row) => <div className="flex items-center justify-end gap-1"><Button variant="ghost" size="icon" className="size-8" onClick={(e) => { e.stopPropagation(); setSelectedItem(row); }} title="Ver detalhes" aria-label={`Ver detalhes de ${row.tipo || "equipamento"} #${row.id}`}><Eye className="size-4" aria-hidden /></Button>{hasRole("Gestor", "Técnico") && <Button variant="ghost" size="icon" className="size-8" onClick={(e) => { e.stopPropagation(); setEditingItem(row); }} title="Editar equipamento" aria-label={`Editar equipamento #${row.id}`}><Pencil className="size-4" aria-hidden /></Button>}</div> },
  ];
