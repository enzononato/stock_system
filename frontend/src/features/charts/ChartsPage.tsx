...
export function ChartsPage() {
  ...
  const [params, setParams] = useState({ year: now.getFullYear(), month: now.getMonth() + 1, revenda: "all" });

  const { revendas = [] } = useConstants();
  const { data: unidades = [] } = useQuery({ queryKey: ["unidades-charts"], queryFn: () => listUnidades() });
  ...
  const { loansChartData, regChartData, totalEmprestimos, totalDevolucoes, totalCadastros } = useMemo(() => {
    if (!isFiltered) {
      ...
      return { loansChartData: lData, regChartData: rData, totalEmprestimos: tEmp, totalDevolucoes: tDev, totalCadastros: tCad };
    }
    ...
    <div className="space-y-6">
      ...
      <div className="surface-panel p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary lg:mr-2 lg:pb-2">
            <Filter className="size-4" aria-hidden />Filtros
          </div>
          ...
          <Button onClick={applyPeriod} disabled={hasInvalidYear} className="w-full sm:w-auto">
            <Calendar className="mr-1.5 size-4" aria-hidden />Aplicar
          </Button>
      ...
      <Section title="Empréstimos × Devoluções" description={`Movimentação diária em ${monthLabel}`}>
        {isLoading ? (
          ...
          <ErrorState error={error} onRetry={() => (isFiltered ? monthly.refetch() : loans.refetch())} />
        ) : loansChartData.every((d) => !d.Empréstimos && !d.Devoluções) ? (
          <EmptyState title="Nenhuma movimentação registrada" description={`Não há empréstimos ou devoluções em ${monthLabel}.`} icon={<PackagePlus className="size-5" aria-hidden />} />
        ) : (
            ...
            <div className="min-w-[560px]">
              <ResponsiveContainer width="100%" height={320}>
                <AreaChart data={loansChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="Empréstimos" stroke="var(--color-chart-1)" strokeWidth={2.5} fill="url(#gradEmprestimos)" dot={{ r: 3 }} activeDot={{ r: 5 }} />
                  <Area type="monotone" dataKey="Devoluções" stroke="var(--color-chart-2)" strokeWidth={2.5} fill="url(#gradDevolucoes)" dot={{ r: 3 }} activeDot={{ r: 5 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <Section title="Novos cadastros" description={`Volume diário de novos patrimônios em ${monthLabel}`}>
        {isLoading ? (
          ...
          <ErrorState error={error} onRetry={() => (isFiltered ? monthly.refetch() : registrations.refetch())} />
        ) : regChartData.every((d) => !d.Cadastros) ? (
          <EmptyState title="Nenhum cadastro registrado" description={`Não há novos cadastros em ${monthLabel}.`} icon={<PackagePlus className="size-5" aria-hidden />} />
        ) : (
          <div className="min-w-0 overflow-x-auto">
            <div className="min-w-[560px]">
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={regChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis allowDecimals={false} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Area type="monotone" dataKey="Cadastros" stroke="var(--color-chart-4)" strokeWidth={2.5} fill="url(#gradCadastros)" dot={{ r: 3 }} activeDot={{ r: 5 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
