...
function BrandMark({ collapsed }: { collapsed: boolean }) {
  return <div className={cn("flex items-center gap-2.5 px-4 py-4", collapsed && "justify-center px-0")}><img src={logoUrl} alt="Revalle" className="size-8 shrink-0 rounded-md object-cover" />{!collapsed && <div className="min-w-0"><p className="truncate text-sm font-bold leading-tight">Controle de Patrimônio</p><p className="truncate text-[11px] text-muted-foreground">Gestão de ativos</p></div>}</div>;
}
