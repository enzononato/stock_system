    ...
    <div className="space-y-6">
      ...
      <Section title="Empréstimos × Devoluções" description={`Movimentação diária em ${monthLabel}`}>
        {loans.isLoading ? <LoadingState label="Carregando indicadores..." /> : loans.error ? <ErrorState error={loans.error} onRetry={() => loans.refetch()} /> : loansChartData.every((d) => !d.Empréstimos && !d.Devoluções) ? <EmptyState title="Nenhuma movimentação registrada" description={`Não há empréstimos ou devoluções em ${monthLabel}.`} icon={<PackagePlus className="size-5" aria-hidden />} /> : (
          <div className="min-w-0 overflow-x-auto"><div className="min-w-[560px]"><ResponsiveContainer width="100%" height={320}><BarChart data={loansChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" className="stroke-border" vertical={false} /><XAxis dataKey="dia" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis allowDecimals={false} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={tooltipStyle} /><Legend wrapperStyle={{ paddingTop: 10, fontSize: 12, fontWeight: 600 }} /><Bar dataKey="Empréstimos" fill="var(--color-chart-1)" radius={[6, 6, 0, 0]} maxBarSize={28} /><Bar dataKey="Devoluções" fill="var(--color-chart-2)" radius={[6, 6, 0, 0]} maxBarSize={28} /></BarChart></ResponsiveContainer></div></div>
        )}
      </Section>
      ...
      <Section title="Novos cadastros" description={`Volume diário de novos patrimônios em ${monthLabel}`}>
        {registrations.isLoading ? <LoadingState label="Carregando indicadores..." /> : registrations.error ? <ErrorState error={registrations.error} onRetry={() => registrations.refetch()} /> : regChartData.every((d) => !d.Cadastros) ? <EmptyState title="Nenhum cadastro registrado" description={`Não há novos cadastros em ${monthLabel}.`} icon={<PackagePlus className="size-5" aria-hidden />} /> : (
          <div className="min-w-0 overflow-x-auto"><div className="min-w-[560px]"><ResponsiveContainer width="100%" height={280}><BarChart data={regChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" className="stroke-border" vertical={false} /><XAxis dataKey="dia" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis allowDecimals={false} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="Cadastros" fill="var(--color-chart-4)" radius={[6, 6, 0, 0]} maxBarSize={32} /></BarChart></ResponsiveContainer></div></div>
        )}
      </Section>
    </div>
  );
